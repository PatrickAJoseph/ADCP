...
int8_t ADCP_Transmit_Queue_Element_Count = 0;                //  Number of elements in transmit queue.
int8_t ADCP_Receive_Queue_Element_Count = 0;				 //  Number of elements in receive queue.	
...
uint8_t ADCP_Transmit_Enqueue(ADCP_Packet_t packet)
{
  ...
  if( !priority )
  {
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[0] = packet.byte[0];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[1] = packet.byte[1];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[2] = packet.byte[2];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[3] = packet.byte[3];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[4] = packet.byte[4];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[5] = packet.byte[5];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[6] = packet.byte[6];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[7] = packet.byte[7];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Back].byte[8] = packet.byte[8];
  }
  else  
  {
	//printf("\nPacket enqueued at back of transmit queue.\n");  
	//printf("\nQueue front position: %d\n", ADCP_Transmit_Queue_Front);	
	
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[0] = packet.byte[0];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[1] = packet.byte[1];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[2] = packet.byte[2];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[3] = packet.byte[3];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[4] = packet.byte[4];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[5] = packet.byte[5];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[6] = packet.byte[6];
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[7] = packet.byte[7];  
	ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[8] = packet.byte[8];	  
  }
...
uint8_t ADCP_Transmit_Dequeue(ADCP_Packet_t* packet)
{
  ...
  {
	packet->byte[0] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[0];  
	packet->byte[1] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[1];  
	packet->byte[2] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[2];  
	packet->byte[3] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[3];  
	packet->byte[4] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[4];  
	packet->byte[5] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[5];  
	packet->byte[6] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[6];  
	packet->byte[7] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[7];  
	packet->byte[8] = ADCP_Transmit_Queue[ADCP_Transmit_Queue_Front].byte[8];  
...
/*
 * @brief:                      Function to print contents of the transmit queue on debug monitor.
 * @parameter:                  None.
 * @return:                     None.
 */

void ADCP_Transmit_Print()
{
  ADCP_Processed_Packet_t processedFrame;

  int8_t position = 0;	

  printf("\nContents of transmit queue:\n\n");	
  ...
  for(i = 0 ; i < ADCP_TX_QUEUE_SIZE ; i++)
  {	  
	for(j = 0 ; j < 9 ; j++)
	{
		printf("%d  ", ADCP_Transmit_Queue[position].byte[j]);
	}
	
	position++;
	position = (position % ADCP_TX_QUEUE_SIZE);
	
	if( i == ADCP_Transmit_Queue_Front )
	{
		printf("\t << FRONT");
	}
	if( i == ADCP_Transmit_Queue_Back )
	{
		printf("\t << BACK");
	}
  ...
  printf("\nTransmit Queue Element Count: %d\n", ADCP_Transmit_Queue_Element_Count);
  printf("Transmit Queue Front Position: %d\n",ADCP_Transmit_Queue_Front);
  printf("Transmit Queue Back Position: %d\n",ADCP_Transmit_Queue_Back);
  printf("Transmit Queue Status: ");
  
  if( ADCP_Transmit_Queue_Status == ADCP_QUEUE_FULL )
  {
	printf("FULL\n");  
  }
  if( ADCP_Transmit_Queue_Status == ADCP_QUEUE_EMPTY )
  {
	printf("EMPTY\n");  
  }
  if( ADCP_Transmit_Queue_Status == ADCP_QUEUE_OK)
  {
	printf("OK\n");  
  }
}
  ...
  if(node_id == ADCP_NODE_ID)
  {
	  ...
	  else
	  {
		if(ADCP_Receive_Queue_Element_Count >= ADCP_TX_QUEUE_SIZE)
		{
			...
			return(ADCP_Receive_Queue_Status = ADCP_QUEUE_FULL);  
		}	  
	  ...
	  if( !priority )
	  {
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[0] = packet.byte[0];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[1] = packet.byte[1];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[2] = packet.byte[2];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[3] = packet.byte[3];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[4] = packet.byte[4];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[5] = packet.byte[5];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[6] = packet.byte[6];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[7] = packet.byte[7];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Back].byte[8] = packet.byte[8];
	  }
	  else  
	  {
		//printf("\nPacket enqueued at back of Receive queue.\n");  
		//printf("\nQueue front position: %d\n", ADCP_Receive_Queue_Front);	
		
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[0] = packet.byte[0];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[1] = packet.byte[1];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[2] = packet.byte[2];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[3] = packet.byte[3];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[4] = packet.byte[4];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[5] = packet.byte[5];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[6] = packet.byte[6];
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[7] = packet.byte[7];  
		ADCP_Receive_Queue[ADCP_Receive_Queue_Front].byte[8] = packet.byte[8];	  
	  }
	  
	  return(ADCP_Receive_Queue_Status = ADCP_QUEUE_OK);
  }
...
uint8_t ADCP_Receive_Dequeue(ADCP_Packet_t* packet)
{
  if( ( ADCP_Receive_Queue_Front == -1 ) && ( ADCP_Receive_Queue_Back == -1 ) )
  {
	return(ADCP_Receive_Queue_Status = ADCP_QUEUE_EMPTY);  
  }
...
void ADCP_Receive_Handler()
{
    ...
    uint8_t status = ADCP_Receive_Dequeue(&packet);
    ADCP_packet_info( &packet, &packetInfo );

    if( status != (uint8_t)ADCP_QUEUE_EMPTY )
    {
